...
import { thinScrollbar, truncateTextUsingEllipsis } from "constants/DefaultTheme";
import type { User } from "constants/userConstants";
import { Button, Icon, Text as NewText, Option, Select, Tooltip } from "design-system";
import { AppIconCollection, Classes, MenuItem as ListItem, Text, TextType } from "design-system-old";
...
import { getTenantPermissions, shouldShowLicenseBanner } from "@appsmith/selectors/tenantSelectors";
...
export const WorkspaceSection = styled.div<{ isMobile?: boolean }>`
  padding: ${({ isMobile }) => (isMobile ? 0 : `0 ${CONTAINER_WRAPPER_PADDING}`)};
...
export const LeftPaneWrapper = styled.div<{ isBannerVisible?: boolean }>`
  ...
  height: ${(props) => (props.isBannerVisible ? `calc(100% - ${props.theme.homePage.header * 2}px)` : "100%")};
  display: flex;
  padding-top: 16px;
  flex-direction: column;
  position: fixed;
  top: calc(${(props) => props.theme.homePage.header}px + ${(props) => (props.isBannerVisible ? 40 : 0)}px);
...
export function Item(props: { label: string; textType: TextType; icon?: string; isFetchingApplications?: boolean }) {
  return (
    <ItemWrapper>
      {props.icon && <StyledIcon name={props.icon} />}
      <Text
        className={!!props.isFetchingApplications ? BlueprintClasses.SKELETON : ""}
...
export function LeftPaneSection(props: {
  ...
  const canCreateWorkspace = getHasCreateWorkspacePermission(isFeatureEnabled, tenantPermissions);
  ...
  return (
    <LeftPaneDataSection isBannerVisible={props.isBannerVisible}>
      <div className="flex items-center justify-between py-3">
        ...
        {canCreateWorkspace && (
          <Tooltip content={createMessage(CREATE_A_NEW_WORKSPACE)} placement="right">
            <Button
...
export function WorkspaceMenuItem({ isFetchingWorkspaces, selected, workspace }: any) {
  ...
  return (
    <ListItem
      ...
      ellipsize={isFetchingWorkspaces ? 100 : 22} /* this is to avoid showing tooltip for loaders */
      icon="group-2-line"
...
export function LeftPane(props: LeftPaneProps) {
  const { activeWorkspaceId, isBannerVisible = false, isFetchingWorkspaces, workspaces = [] } = props;
...
export const ApplicationsWrapper = styled.div<{
  ...
  ${({ isBannerVisible, isMobile }) => (isBannerVisible ? (isMobile ? "margin-top: 78px;" : "margin-top: 48px;") : "")}
  ${({ isMobile }) =>
    isMobile ? `padding: ${CONTAINER_WRAPPER_PADDING} 0;` : `padding:  0 0 ${CONTAINER_WRAPPER_PADDING};`}
...
export function ApplicationsSection(props: any) {
  const { activeWorkspaceId, applications, packages, workflows, workspaces } = props;
  ...
  const [workspaceToOpenMenu, setWorkspaceToOpenMenu] = useState<string | null>(null);
  const isManageEnvironmentEnabled = useSelector(allowManageEnvironmentAccessForUser);
  const updateApplicationDispatch = (id: string, data: UpdateApplicationPayload) => {
    dispatch(updateApplication(id, data));
  };
  const isLoadingResources = isFetchingWorkspaces || isFetchingApplications || isFetchingPackages;
  ...
  const [selectedWorkspaceIdForImportApplication, setSelectedWorkspaceIdForImportApplication] = useState<
    string | undefined
  >();
  ...
  }, [openImportModal, activeWorkspaceId, setSelectedWorkspaceIdForImportApplication]);
  ...
  function WorkspaceMenuTarget(props: { workspaceName: string; disabled?: boolean; workspaceSlug: string }) {
    const { disabled, workspaceName, workspaceSlug } = props;

    return (
      <WorkspaceNameWrapper className="t--workspace-name-text" disabled={disabled}>
        <StyledAnchor id={workspaceSlug} />
        <WorkspaceNameHolder className={isLoadingResources ? BlueprintClasses.SKELETON : ""} type={TextType.H4}>
          <WorkspaceNameElement className={isLoadingResources ? BlueprintClasses.SKELETON : ""} isMobile={isMobile}>
  ...
  const createNewApplication = (applicationName: string, workspaceId: string) => {
    const color = getRandomPaletteColor(theme.colors.appCardColors);
    const icon = AppIconCollection[Math.floor(Math.random() * AppIconCollection.length)];
  ...
  function NoWorkspaceFound() {
    return (
      <div className="flex flex-col items-center justify-center mt-[180px]">
        <img className="mb-6" src={`${getAssetUrl(`${ASSETS_CDN_URL}/no-workspace-found.svg`)}`} />
  ...
  const activeWorkspace = workspaces.find((workspace: Workspace) => workspace.id === activeWorkspaceId);
  if (!activeWorkspace && !isFetchingWorkspaces) return <NoWorkspaceFound />;

  if (!activeWorkspace) return null;

  let workspacesListComponent;
  if (!isLoadingResources && props.searchKeyword && props.searchKeyword.trim().length > 0 && workspaces?.length === 0) {
    workspacesListComponent = (
      ...
      >
        <CreateNewLabel type={TextType.H4}>{createMessage(NO_APPS_FOUND)}</CreateNewLabel>
    ...
    const canInviteToWorkspace = isPermitted(activeWorkspace.userPermissions, PERMISSION_TYPE.INVITE_USER_TO_WORKSPACE);
    const canDeleteWorkspace = hasDeleteWorkspacePermission(activeWorkspace?.userPermissions || []);
    const hasCreateNewApplicationPermission = hasCreateNewAppPermission(activeWorkspace.userPermissions) && !isMobile;
    const renderManageEnvironmentMenu =
      isManageEnvironmentEnabled && hasManageWorkspaceEnvironmentPermission(activeWorkspace.userPermissions);
        ...
        <WorkspaceSection className="t--workspace-section" isMobile={isMobile}>
          <WorkspaceDropDown isMobile={isMobile}>
            ...
            {selectedWorkspaceIdForImportApplication && (
              <ImportModal
                isModalOpen={selectedWorkspaceIdForImportApplication === activeWorkspace.id}
            ...
            {!isLoadingResources && (
              <WorkspaceShareUsers>
                ...
                <WorkspaceAction
                  ...
                  setSelectedWorkspaceIdForImportApplication={setSelectedWorkspaceIdForImportApplication}
                  workspace={activeWorkspace}
                  workspaceId={activeWorkspaceId}
                />
                {(currentUser || isLoadingResources) && !isMobile && showWorkspaceMenuOptions && (
                  <WorkspaceMenu
                    canDeleteWorkspace={
                      applications.length === 0 && packages.length === 0 && workflows.length === 0 && canDeleteWorkspace
                    }
  ...
  return (
    <ApplicationContainer className="t--applications-container" isMobile={isMobile}>
      {workspacesListComponent}
...
export const ApplictionsMainPage = (props: any) => {
  ...
  if (!isFetchingWorkspaces) {
    ...
    workspaces = loadingUserWorkspaces.map((loadingWorkspaces) => loadingWorkspaces.workspace) as any;
  }
  ...
  useEffect(() => {
    setActiveWorkspaceId(workspaceIdFromQueryParams ? workspaceIdFromQueryParams : workspaces[0]?.id);
    if (activeWorkspaceId && fetchedWorkspaceId && fetchedWorkspaceId !== activeWorkspaceId) {
      const activeWorkspace: Workspace = workspaces.find((workspace: Workspace) => workspace.id === activeWorkspaceId);
      if (activeWorkspace) {
        dispatch({
  ...
  const workflowsOfWorkspace = activeWorkspaceId
    ? fetchedWorkflows.filter((workflow) => workflow.workspaceId === activeWorkspaceId)
    : [];
  ...
  return (
    <PageWrapper displayName="Applications">
      ...
      <MediaQuery maxWidth={MOBILE_MAX_WIDTH}>
        {(matches: boolean) => (
          <ApplicationsWrapper isBannerVisible={!!isBannerVisible} isMobile={matches}>
            {!isFetchingWorkspaces && matches && (
              <WorkspaceSelectorWrapper>
                <Select onSelect={(val) => history.push(`/applications?workspaceId=${val}`)} value={activeWorkspaceId}>
...
export interface ApplicationProps {
  ...
  getAllWorkspaces: (params: { fetchEntities: boolean; workspaceId: string | null }) => void;
  workspaces: any;
  currentUser?: User;
  searchKeyword: string | undefined;
  setHeaderMetaData: (hideHeaderShadow: boolean, showHeaderSeparator: boolean) => void;
  resetEditor: () => void;
...
export class Applications<Props extends ApplicationProps, State extends ApplicationState> extends Component<
  Props,
  State
> {
  ...
  public render() {
    if (this.props.currentApplicationIdForCreateNewApp) {
      ...
      return (
        <CreateNewAppsOption currentApplicationIdForCreateNewApp={this.props.currentApplicationIdForCreateNewApp} />
      );
    } else {
      return (
        <ApplictionsMainPage
...
export const mapStateToProps = (state: AppState) => ({
  ...
  currentApplicationIdForCreateNewApp: getCurrentApplicationIdForCreateNewApp(state),
  currentWorkspaceId: getCurrentWorkspaceId(state),
  isReconnectModalOpen: getIsReconnectingDatasourcesModalOpen(state),
});

export const mapDispatchToProps = (dispatch: any) => ({
  getAllWorkspaces: ({ fetchEntities, workspaceId }: { fetchEntities: boolean; workspaceId: string | null }) => {
  ...
  setHeaderMetaData: (hideHeaderShadow: boolean, showHeaderSeparator: boolean) => {
